...
下载 LCQMC ... → data/lcqmc/（开发者自主练习）
下载 BQ Corpus ... → data/bq_corpus/（开发者自主练习）
```
...
### 4.3 参数建议（快速演示 vs 完整练习）

| 场景 | 推荐参数 |
|------|---------|
| 快速演示 | `--num_hidden_layers 4 --epochs 3 --batch_size 32` |
| 开发者完整训练 | `--num_hidden_layers 12 --epochs 5 --batch_size 16` |
...
- CrossEncoder Accuracy 最高但 F1 最低：1 epoch 训练不足时倾向于预测多数类（负类），
 accuracy 因此虚高——这本身就是一个点（accuracy ≠ F1）
...
> **类别平衡说明**：AFQMC 正负比 31:69。`train_sft.py` 默认开启**正负平衡采样**（各取 `num_train//2` 条），避免模型退化为全预测负例（F1=0 的反例在第一版实测中出现过，与 CrossEncoder 1-epoch 是同一问题）。
