...
**项目目标：** 对比"表示型"与"交互型"两种 BERT 文本匹配范式，以及 LLM zero-shot 和 LLM SFT（LoRA 指令微调），形成判别式 vs 生成式的完整对比视角。
...
- 正负比 31:69，接近真实业务分布，有类别不均衡参考价值
- LCQMC / BQ Corpus 保留为开发者自主练习数据集
...
**层数限制：** 默认 4 层（快速跑通），全量 12 层留给开发者自行对比
...
- 进阶（开发者练习）：Online Hard Negative Mining，同 batch 内选最难负样本
...
这条对比直接给出"训练不足 → Accuracy 虚高 / F1 偏低"的样本，**accuracy 并不总能反映模型真实能力**，尤其在类别不均衡 + 训练不足时。
...
| 开发者消融实验 | 4/12 层 × 1/5 epoch 的 2×2 对比 | 代码无需改动，只改命令行参数 |
